...
def compute_controlnet_weighting(control, cnet):
    ...
    for k, v in control.items():
        for i in range(len(v)):
            ...
            # ---------- FIXED batch size for MultiDiffusion + ControlNet ----------
            # control_signal: [B_control, C, H, W]
            # final_weight: [B_weight]
            # Here in Forge, it's common: B_control = tile_batch_size (e.g., 4),
            # B_weight = 2 (cond/uncond)
            # Batch sizes need to be aligned, like ComfyUI does.
            if isinstance(final_weight, torch.Tensor):
                bs_control = control_signal.shape[0]
                bs_weight = final_weight.shape[0]

                if bs_control != bs_weight:
                    # If the scale size is smaller, we repeat them along the batch axis
                    if bs_weight == 1:
                        final_weight = final_weight.repeat(bs_control)
                    elif bs_control > bs_weight and bs_control % bs_weight == 0:
                        repeat_factor = bs_control // bs_weight
                        final_weight = final_weight.repeat(repeat_factor)
                    # If the opposite is true, we repeat control_signal (less typical case,
                    # but this makes the behavior consistent)
                    elif bs_weight > bs_control and bs_weight % bs_control == 0:
                        repeat_factor = bs_weight // bs_control
                        control_signal = control_signal.repeat(repeat_factor, 1, 1, 1)
                        B, C, H, W = control_signal.shape  # обновляем B
                    # In very uneven cases, we try to brutally fit the smaller one to the larger one
                    else:
                        if bs_control > bs_weight:
                            reps = (bs_control + bs_weight - 1) // bs_weight
                            final_weight = final_weight.repeat(reps)[:bs_control]
                        else:
                            reps = (bs_weight + bs_control - 1) // bs_control
                            control_signal = control_signal.repeat(reps, 1, 1, 1)[:bs_weight]
                            B, C, H, W = control_signal.shape
            # ---------- end of batch-size fix ----------
            ...
            if isinstance(advanced_mask_weighting, torch.Tensor):
                ...
                control_signal = control_signal * torch.nn.functional.interpolate(
                    advanced_mask_weighting.to(control_signal), size=(H, W), mode='bilinear'
                )
