...
def load_torch_file(ckpt, safe_load=False, device=None):
    ...
    def try_load(load_fn):
        try:
            return load_fn(device)
        except (RuntimeError, torch.cuda.OutOfMemoryError) as e:
            print(f"⚠️ Error loading on {device}: {e}. Switching to CPU.")
            return load_fn(torch.device("cpu"))

    if ckpt.lower().endswith(".safetensors"):
        def load_safetensors(dev):
            return safetensors.torch.load_file(ckpt, device=dev.type)
        sd = try_load(load_safetensors)
        ...
        def load_pt(dev):
            ...
            if safe_load_flag:
                pl_sd = torch.load(ckpt, map_location=dev, weights_only=True)
            else:
                pl_sd = torch.load(ckpt, map_location=dev, pickle_module=backend.misc.checkpoint_pickle)
