...
def free_memory(memory_required, device, keep_loaded=[], free_all=False, model_changed=False, ignore_vram_offload=False, strategy_override=None):
    from modules import shared
    strategy = strategy_override or shared.opts.memory_cleanup_strategy
    #print(f"[UI] Updated strategy to: {strategy}") #debug info

    if strategy == "Smart Purge":
        _free_memory_smartpurge(memory_required, device, keep_loaded, free_all, model_changed, ignore_vram_offload)
        return

    elif strategy == "Smart":
        _free_memory_smart(memory_required, device, keep_loaded, free_all, model_changed, ignore_vram_offload)
        return

    elif strategy == "Always":
        _free_memory_clear_always(memory_required, device, keep_loaded, free_all=True)
        return

    elif strategy == "Full":
        _free_memory_clear_full(memory_required, device, keep_loaded, free_all=True)
        return

    elif strategy == "Soft (Cache)":
        _free_memory_soft_cache(memory_required, device, keep_loaded=[], free_all=False, model_changed=False, ignore_vram_offload=False)
        return

    elif strategy == "GPU Cache":
        _free_memory_gpu_cache(memory_required, device, keep_loaded=[], free_all=False, model_changed=False, ignore_vram_offload=False)
        return

    elif strategy == "None (Upscale)":
        _free_memory_null()
        return
...
def _free_memory_smartpurge(memory_required, device, keep_loaded=[], free_all=False, model_changed=False, ignore_vram_offload=False):
    ...
    for i in range(len(current_loaded_models) - 1, -1, -1):
        model_ref = current_loaded_models[i]
        if sys.getrefcount(model_ref.model) <= 2:
            current_loaded_models.pop(i).model_unload(avoid_model_moving=True)
            unloaded_model = True

    # If the model has changed, we clear all memory
    if model_changed:
    ...
    if ignore_vram_offload:
        offload_everything = vram_state == VRAMState.NO_VRAM
    else:
        offload_everything = ALWAYS_VRAM_OFFLOAD or vram_state == VRAMState.NO_VRAM

    # Main loop for unloading models if they are not in the keep_loaded list and are on the correct device
    for i in range(len(current_loaded_models) - 1, -1, -1):
...
def _free_memory_smart(memory_required, device, keep_loaded=[], free_all=False, model_changed=False, ignore_vram_offload=False):
    unloaded_model = False
...
def _free_memory_clear_always(memory_required, device, keep_loaded=[], free_all=True):
    unloaded_model = False
    ...
    if free_all:
        memory_required = float('inf')
    else:
        offload_everything = ALWAYS_VRAM_OFFLOAD

        # Unload models not in keep_loaded
        for i in range(len(current_loaded_models) - 1, -1, -1):
            shift_model = current_loaded_models[i]
            if shift_model.device == device and shift_model not in keep_loaded:
                m = current_loaded_models.pop(i)
...
def _free_memory_soft_cache(memory_required, device, keep_loaded=[], free_all=False, model_changed=False, ignore_vram_offload=False):
    soft_empty_cache()
    return
...
def free_memory_strategy(strategy: str):
    print(f"[Memory] Applying strategy: {strategy}")

    device = normalize_device("cuda")

    free_memory(
        memory_required=0,
        device=device,
        keep_loaded=[],
        free_all=(strategy == "Clear Always"),
        model_changed=(strategy == "Clear Always"),
        ignore_vram_offload=(strategy == "Native Forge"),
        strategy_override=strategy
    )


def unload_all_models():
    free_memory(1e30, get_torch_device(), free_all=False)
