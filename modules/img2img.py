...
def process_batch(p, input, output_dir, inpaint_mask_dir, args,
                  to_scale=False, scale_by=1.0, use_png_info=False,
    ...
    for i, image in enumerate(batch_images):
        ...
        # create a local p for each image
        p_local = copy.copy(p)
        
        # reset fields that must be unique
        p_local.init_images = [img]
        p_local.image_mask = None
        p_local.batch_size = 1
        p_local.width = img.width
        p_local.height = img.height
        if to_scale:
            p_local.width = int(img.width * scale_by)
            p_local.height = int(img.height * scale_by)
        else:
            p_local.width = img.width
            p_local.height = img.height
        ...
        if use_png_info:
            ...
            p_local.prompt = prompt + (" " + parsed_parameters["Prompt"] if "Prompt" in parsed_parameters else "")
            p_local.negative_prompt = negative_prompt + (" " + parsed_parameters["Negative prompt"] if "Negative prompt" in parsed_parameters else "")
            p_local.seed = int(parsed_parameters.get("Seed", seed))
            p_local.cfg_scale = float(parsed_parameters.get("CFG scale", cfg_scale))
            p_local.sampler_name = parsed_parameters.get("Sampler", sampler_name)
            p_local.steps = int(parsed_parameters.get("Steps", steps))

            model_info = get_closet_checkpoint_match(parsed_parameters.get("Model hash", None))
            if model_info is not None:
                p_local.override_settings['sd_model_checkpoint'] = model_info.name
            elif sd_model_checkpoint_override:
                p_local.override_settings['sd_model_checkpoint'] = sd_model_checkpoint_override
            else:
                p_local.override_settings.pop("sd_model_checkpoint", None)

        if output_dir:
            p_local.outpath_samples = output_dir
            p_local.override_settings['save_to_dirs'] = False

        if opts.img2img_batch_use_original_name:
            filename_pattern = f'{image_path.stem}-[generation_number]' if p_local.n_iter > 1 or p_local.batch_size > 1 else f'{image_path.stem}'
            p_local.override_settings['samples_filename_pattern'] = filename_pattern

        proc = modules.scripts.scripts_img2img.run(p_local, *args)
        if proc is None:
            proc = process_images(p_local)
        ...
        if not discard_further_results and proc:
            if batch_results:
                ...
                batch_results = proc
                batch_results.images = []  # не держим картинки в памяти

            if 0 <= shared.opts.img2img_batch_show_results_limit < len(batch_results.infotexts):
