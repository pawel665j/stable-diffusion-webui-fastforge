...
def get_align_your_steps_sigmas(n, sigma_min, sigma_max, device, model_type="SD1", denoise=1.0):
    """
    Forge SD replacement for ComfyUI AlignYourStepsScheduler.
    Recreates the exact sigma schedule used by ComfyUI:
    - uses fixed NOISE_LEVELS table (SD1 / SDXL / SVD)
    - applies log-linear interpolation when steps mismatch
    - trims sigma curve according to denoise
    - ensures last sigma = 0
    """

    # --- Noise tables from ComfyUI AlignYourStepsScheduler ---
    NOISE_LEVELS = {
        "SD1":  [14.6146412293, 6.4745760956, 3.8636745985, 2.6946151520, 1.8841921177,
                 1.3943805092, 0.9642583904, 0.6523686016, 0.3977456272, 0.1515232662, 0.0291671582],

        "SDXL": [14.6146412293, 6.3184485287, 3.7681790315, 2.1811480769, 1.3405244945,
                 0.8620721141, 0.5550693289, 0.3798540708, 0.2332364134, 0.1114188177, 0.0291671582],

        "SVD":  [700.00, 54.5, 15.886, 7.977, 4.248,
                 1.789, 0.981, 0.403, 0.173, 0.034, 0.002]
    }

    # --- Helper: log-linear interpolation (same as ComfyUI) ---
    ...
    # --- Select noise curve based on model type ---
    base_sigmas = NOISE_LEVELS.get(model_type, NOISE_LEVELS["SD1"])[:]

    # --- Interpolate if step count mismatches ---
    if (n + 1) != len(base_sigmas):
        sigmas = loglinear_interp(base_sigmas, n + 1)
    else:
        sigmas = np.array(base_sigmas, dtype=np.float32)

    # --- Apply denoise trimming (same as ComfyUI) ---
    total_steps = n
    if denoise < 1.0:
        if denoise <= 0.0:
            return torch.FloatTensor([]).to(device)
        total_steps = round(n * denoise)

    # Keep only last (total_steps + 1) sigmas
    sigmas = sigmas[-(total_steps + 1):]

    # --- Force last sigma = 0 (required by k-diffusion) ---
    sigmas[-1] = 0.0
