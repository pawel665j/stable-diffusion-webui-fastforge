...
options_templates.update(options_section(('saving-images', "Saving images/grids", "saving"), {
    ...
    "jpeg_quality": OptionInfo(100, "Quality for saved jpeg and avif images", gr.Slider, {"minimum": 1, "maximum": 100, "step": 1}),
    "webp_lossless": OptionInfo(False, "Use lossless compression for webp images"),
    "export_for_4chan": OptionInfo(False, "Save copy of large images as JPG").info("if the file size is above the limit, or either width or height are above the limit"),
    "img_downscale_threshold": OptionInfo(100.0, "File size limit for the above option, MB", gr.Number),
    "target_side_length": OptionInfo(10000, "Width/height limit for the above option, in pixels", gr.Number),
    ...
    "save_write_log_csv": OptionInfo(False, "Write log.csv when saving images using 'Save' button"),
    ...
    "notification_audio": OptionInfo(False, "Play notification sound after image generation").info("notification.mp3 should be present in the root directory").needs_reload_ui(),
...
options_templates.update(options_section(('upscaling', "Upscaling", "postprocessing"), {
    "ESRGAN_tile": OptionInfo(512, "Tile size for ESRGAN upscalers.", gr.Slider, {"minimum": 0, "maximum": 2048, "step": 16}).info("0 = no tiling"),
    "ESRGAN_tile_overlap": OptionInfo(128, "Tile overlap for ESRGAN upscalers.", gr.Slider, {"minimum": 0, "maximum": 512, "step": 1}).info("Low values = visible seam"),
    "composite_tiles_on_gpu": OptionInfo(True, "Composite the Tiles on GPU").info("improve performance and resource utilization"),
    ...
    "DAT_tile": OptionInfo(512, "Tile size for DAT upscalers.", gr.Slider, {"minimum": 0, "maximum": 2048, "step": 16}).info("0 = no tiling"),
    "DAT_tile_overlap": OptionInfo(128, "Tile overlap for DAT upscalers.", gr.Slider, {"minimum": 0, "maximum": 512, "step": 1}).info("Low values = visible seam"),
...
options_templates.update(options_section(('system', "System", "system"), {
    ...
    "memmon_poll_rate": OptionInfo(10, "VRAM usage polls per second during generation.", gr.Slider, {"minimum": 0, "maximum": 40, "step": 1}).info("0 = disable"),
...
options_templates.update(options_section(('sd', "Stable Diffusion", "sd"), {
    ...
    "enable_quantization": OptionInfo(True, "Enable quantization in K samplers for sharper and cleaner results. This may change existing seeds").needs_reload_ui(),
    ...
    "comma_padding_backtrack": OptionInfo(74, "Prompt word wrap length limit", gr.Slider, {"minimum": 0, "maximum": 74, "step": 1}).info("in tokens - for texts shorter than specified, if they don't fit into 75 token limit, move them to the next 75 token chunk"),
    "sdxl_clip_l_skip": OptionInfo(True, "Clip skip SDXL", gr.Checkbox).info("Enable Clip skip for the secondary clip model in sdxl. Has no effect on SD 1.5 or SD 2.0/2.1."),
    "CLIP_stop_at_last_layers": OptionInfo(1, "(Managed by Forge)", gr.State, infotext="Clip skip"),
    "upcast_attn": OptionInfo(False, "Upcast cross attention layer to float32"),
    "randn_source": OptionInfo("NV", "Random number generator source.", gr.Radio, {"choices": ["GPU", "CPU", "NV"]}, infotext="RNG").info("changes seeds drastically; use CPU to produce the same picture across different videocard vendors; use NV to produce same picture as on NVidia videocards"),
    "tiling": OptionInfo(False, "Tiling", infotext='Tiling').info("produce a tileable picture"),
    "hires_fix_refiner_pass": OptionInfo("first pass", "Hires fix: which pass to enable refiner for", gr.Radio, {"choices": ["first pass", "second pass", "both passes"]}, infotext="Hires refiner"),
    "show_mahiro": OptionInfo(True, "Display the MaHiRo Toggle").info('see <a href="https://huggingface.co/spaces/yoinked/blue-arxiv">blue-arxiv</a> - <b>id:</b> <ins>2024-1208.1</ins>').needs_reload_ui(),
...
"""),
    "sd_vae_checkpoint_cache": OptionInfo(1, "VAE Checkpoints to cache in RAM", gr.Slider, {"minimum": 0, "maximum": 10, "step": 1}),
    "sd_vae": OptionInfo("Automatic", "(Managed by Forge)", gr.State, infotext='VAE'),
    "sd_vae_overrides_per_model_preferences": OptionInfo(False, "Selected VAE overrides per-model preferences").info("you can set per-model VAE either by editing user metadata for checkpoints, or by making the VAE have same name as checkpoint"),
    "auto_vae_precision_bfloat16": OptionInfo(True, "Automatically convert VAE to bfloat16").info("triggers when a tensor with NaNs is produced in VAE; disabling the option in this case will result in a black square image; if enabled, overrides the option below"),
    "auto_vae_precision": OptionInfo(False, "Automatically revert VAE to 32-bit floats").info("triggers when a tensor with NaNs is produced in VAE; disabling the option in this case will result in a black square image"),
...
options_templates.update(options_section(('optimizations', "Optimizations", "sd"), {
    ...
    "pad_cond_uncond": OptionInfo(True, "Pad prompt/negative prompt", infotext='Pad conds').info("improves performance when prompt and negative prompt have different lengths; changes seeds"),
...
options_templates.update(options_section(('ui_alternatives', "UI alternatives", "ui"), {
    ...
    "interrupt_after_current": OptionInfo(False, "Don't Interrupt in the middle").info("when using Interrupt button, if generating more than one image, stop after the generation of an image has finished, instead of immediately"),
}))
...
"""),
    ...
    "add_model_hash_to_info": OptionInfo(False, "Add model hash to infotext"),
    "add_vae_name_to_info": OptionInfo(True, "Add VAE name to infotext"),
    "add_vae_hash_to_info": OptionInfo(False, "Add VAE hash to infotext"),
    "add_user_name_to_info": OptionInfo(False, "Add user name to infotext when authenticated"),
    "add_version_to_infotext": OptionInfo(False, "Add program version to infotext"),
...
options_templates.update(options_section(('ui', "Live previews", "ui"), {
    ...
    "show_progress_type": OptionInfo("Approx cheap", "Live preview method", gr.Radio, {"choices": ["Approx NN", "Approx cheap", "TAESD"]}).info("Approx NN: fast preview; TAESD = high-quality preview; Approx cheap = fastest but low-quality preview"),
    ...
    "live_preview_fast_interrupt": OptionInfo(True, "Return image with chosen live preview method on interrupt").info("makes interrupts faster"),
...
options_templates.update(options_section(('sampler-params', "Sampler parameters", "sd"), {
    ...
    'sgm_noise_multiplier': OptionInfo(True, "SGM noise multiplier", infotext='SGM noise multiplier').link("PR", "https://github.com/AUTOMATIC1111/stable-diffusion-webui/pull/12818").info("Match initial noise to official SDXL implementation - only useful for reproducing images"),
...
options_templates.update(options_section(('memory', "Memory", "memory_cleanup"), {
    "lora_computation_strategy_explanation": OptionHTML("""
LoRa computation cast:
<table>
  <tr>
    <th>Method</th>
    <th>Overview</th>
  ...
  <tr>
    <td><b>Auto-HW</b></td>
    <td>GPU-based bfloat16 or float32</td>
  </tr>
  <tr>
    <td><b>Autocast</b></td>
    <td>CUDA-based bfloat16 or float32</td>
  </tr>
  <tr>
    <td><b>bfloat16 / float16</b></td>
    <td>Low VRAM, but cost of LoRa first run.</td>
  </tr>
  <tr>
    <td><b>float32</b></td>
    <td>Faster, but heavy VRAM usage.</td>
  </tr>
</table>
"""),
    "lora_computation_strategy": OptionInfo(
        "Auto HW",
        "LoRa Computation Strategy",
        gr.Dropdown,
        {"choices": ["Auto HW", "Autocast", "float16", "bfloat16", "float32"]},
        section=("memory", "Memory", "memory_cleanup")
    ),
	"memory_cleanup_strategy_explanation": OptionHTML("""
...
<table>
  ...
  <tr>
    <td><b>Smart Purge</b></td>
    <td>Same as Smart. But always purges GPU cache before main logic. Also, previous models will be cleared every generation start.</td>
  </tr>
  <tr>
  <tr>
  <td><b>Smart</b></td>
  <td>
    <ul>
      <li> if the model is not used anywhere else (only in the list and locally), it will be unloaded.</li>
      <li> if the model changes, all memory will be cleaned.</li>
      <li> if the model is not in the keep_loaded list and is on the desired device, it will be unloaded.</li>
      <li> if at least one model was unloaded, the cache will be cleared.</li>
      <li> if the model was not unloaded from memory and if it did not change, it will be saved in memory.</li>
      <li> cache will be cleaned at each generation.</li>
      <li> previous models will be cleared every generation start.</li>
    </ul>
  </td>
  </tr>
  <tr>
    <td><b>Always</b></td>
    <td>Always clear all possible things from RAM+VRAM. Also, previous models will be cleared every generation start.</td>
  </tr>
  <tr>
    <td><b>Full</b></td>
    <td>Always purges GPU cache and then clear all possible things from RAM+VRAM. Also, previous models will be cleared every generation start.</td>
  </tr>  
  <tr>
    <td><b>Soft (Cache)</b></td>
    <td>Clearing only cache at run (softcache clear). Also, previous models will be cleared every generation start.</td>
  </tr>  
  <tr>
    <td><b>GPU Cache</b></td>
    <td>Clearing only GPU cache at run. Also, previous models will be cleared every generation start.</td>
  </tr>
  <tr>
    <td><b>None (Upscale)</b></td>
    <td>No memory clearing, use while upscale. Also, previous models will be cleared every generation start.</td>
  </tr>
</table>
"""),
    "memory_cleanup_strategy": OptionInfo(
        "Smart Purge",
        "Memory cleanup strategy",
        gr.Dropdown,
