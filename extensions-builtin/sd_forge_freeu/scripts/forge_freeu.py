...
class FreeUForForge(scripts.Script):
    ...
    presets_builtin = [
        ...
        ('Forge default', 1.05, 1.08, 0.95, 0.80, 0.0, 1.0),
        ('Forge stable', 1.01, 1.02, 0.99, 0.95, 0.0, 1.0),
    ...
    def ui(self, *args, **kwargs):
        with gr.Accordion(open=False, label="FreeU", elem_id="freeu-integrated"):
            freeu_enabled = gr.Checkbox(label="Enabled", value=False)

            with gr.Row():
                freeu_b1 = gr.Slider(label='B1', minimum=0, maximum=2, step=0.01, value=1.05)
                freeu_b2 = gr.Slider(label='B2', minimum=0, maximum=2, step=0.01, value=1.08)
            with gr.Row():
                freeu_s1 = gr.Slider(label='S1', minimum=0, maximum=4, step=0.01, value=0.95)
                freeu_s2 = gr.Slider(label='S2', minimum=0, maximum=4, step=0.01, value=0.80)
            with gr.Row():
            ...
            with gr.Row():
                freeu_preset = gr.Dropdown(label='', choices=[x[0] for x in FreeUForForge.presets],
                                           value='(presets)', type='index', scale=0, allow_custom_value=True)
        ...
        freeu_preset.input(fn=setParams,
                           inputs=[freeu_preset],
                           outputs=[freeu_b1, freeu_b2, freeu_s1, freeu_s2, freeu_start, freeu_end, freeu_preset],
                           show_progress=False)
