...
class ControlNetUiGroup(object):
    ...
    def render(self, tabname: str, elem_id_tabname: str) -> None:
        ...
        self.enabled.change(
            lambda enabled: gr.update(value=enabled),
            inputs=[self.enabled],
        ...
        self.enabled.change(
            ...
            _js=f"""
                (enabled) => {{
                    const acc = document.querySelector("#{self.accordion.elem_id}");
                    if (!acc) return;

                    const labelWrap = acc.querySelector(".label-wrap");
                    if (!labelWrap) return;

                    const headerSpan = labelWrap.querySelector("span.svelte-1w6vloh");
                    if (!headerSpan) return;

                    headerSpan.classList.toggle("cnet-unit-active", !!enabled);
                }}
            """,
            inputs=[self.enabled],
            outputs=[]
