# Dynamic CFG — unified single file
# Combines: dynthres_core.py + dynthres.py + forge_dynamic_thresholding.py
...
class DynThresh:
    ...
    def interpret_scale(self, scale, mode, min_val):
        scale -= min_val
        max_val = self.max_steps - 1
        frac = self.step / max_val
        if mode == "Constant":
            pass
        ...
        if self.experiment_mode == 1:
            ...
            if self.step / (self.max_steps - 1) > 0.2:
                ...
                if max_rgb > 2.4 and max_w > 3.0:
                    res_rgb /= max_rgb / 2.4
            actual_res = torch.einsum("laxy,ab->lbxy", res_rgb, torch.inverse(coefs))

        return actual_res


######################### DynamicThresholding Node #########################
...
class DynamicThresholdingNode:
    ...
    def patch(self, model, mimic_scale, threshold_percentile, mimic_mode,
              mimic_scale_min, cfg_mode, cfg_scale_min, sched_val,
              separate_feature_channels, scaling_startpoint,
              variability_measure, interpolate_phi):
        dynamic_thresh = DynThresh(mimic_scale, threshold_percentile, mimic_mode,
                                   mimic_scale_min, cfg_mode, cfg_scale_min,
                                   sched_val, 0, 999,
                                   separate_feature_channels == "enable",
                                   scaling_startpoint, variability_measure,
                                   interpolate_phi)
        def sampler_dyn_thresh(args):
            x = args["input"]
            cond = x - args["cond"]
            uncond = x - args["uncond"]
            cond_scale = args["cond_scale"]
            # sigma -> timestep -> invert to 0..999 order
            time_step = model.model.predictor.timestep(args["sigma"])
            time_step = time_step[0].item()
            dynamic_thresh.step = 999 - time_step
            return x - dynamic_thresh.dynthresh(cond, uncond, cond_scale, None)
...
class DynamicThresholdingForForge(scripts.Script):
    ...
    def ui(self, *args, **kwargs):
        with gr.Accordion(open=False, label=self.title()):
            ...
            separate_feature_channels = gr.Radio(label='Separate Feature Channels', choices=['enable', 'disable'], value='enable')
            scaling_startpoint = gr.Radio(label='Scaling Startpoint', choices=DynThresh.Startpoints, value='MEAN')
            variability_measure = gr.Radio(label='Variability Measure', choices=DynThresh.Variabilities, value='AD')
        ...
        self.infotext_fields = [
            ...
            (enabled, "dynthres_enabled"),  # ← строковый ключ для поиска
        ...
        return (
            enabled, mimic_scale, threshold_percentile, mimic_mode, mimic_scale_min,
    ...
    def process_before_every_sampling(self, p, *script_args, **kwargs):
        enabled, mimic_scale, threshold_percentile, mimic_mode, mimic_scale_min, cfg_mode, cfg_scale_min, \
            sched_val, separate_feature_channels, scaling_startpoint, variability_measure, interpolate_phi = script_args
